...
C# Usage
	Only use the C# version if you have to! C# code is fully compatible with the LeanTween.js version(as long as it is placed in the Plugins directory). The C# version is not as up to date as the JS version. The C# version is only helpful if you compile your code to a DLL, otherwise you should use the LeanTween.js file. To use the C# version unzip the file LeanTween.cs.zip and delete LeanTween.js.
