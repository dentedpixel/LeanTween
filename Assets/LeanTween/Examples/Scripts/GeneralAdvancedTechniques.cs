...
public class GeneralAdvancedTechniques : MonoBehaviour {
	...
	void Start () {
		...
		for(int i = 0; i < avatarSpeed.Length; i++)
			LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(0.2f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any time you set the speed it overrides the time value
	
		// move around a circle at a constant speed
		for(int i = 0; i < avatarSpeed2.Length; i++){
			LeanTween.moveLocal( avatarSpeed2[i], i == 0 ? circleSm : circleLrg, 1f).setSpeed(0.05f).setRepeat(-1);
		}
